...
//			dto.setPw(rs.getString("pw"));	 // 비밀번호 변경은 나중에 따로 빼내서 구현
	...
	public String findId(StudentDTO dto) {
		String sql = "select id from student where name=? and phone=?";
		
		String id = jt.queryForObject(sql, String.class, dto.getName(), dto.getPhone());
	...
	public int updatePw(StudentDTO dto) {
		String sql = "update student set pw=? where id=? and name=? and phone=?";
		int row = jt.update(sql, dto.getPw(), dto.getId(), dto.getName(), dto.getPhone());
