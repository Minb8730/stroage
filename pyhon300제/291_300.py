...
# f = open("C:\\Users\\user\\Desktop\\programming매수종목1.txt", mode="wt", encoding="utf-8")
# f.write("005930\n")
# f.write("005380\n")
# f.write("035420")
# f.close()
...
# f = open("C:\\Users\\user\\Desktop\\programming매수종목2.txt", mode = "wt", encoding= "utf-8")
# f.write("005930 삼성전자\n")
# f.write("005380 현대차\n")
...
#293 CSV 파일 쓰기    -> 텍스트에서 값들 사이에 ,콤마로 띄어준다.   Excel 파일로 다룰 때 유용하다.  
# text 파일이 아닌 오디오 비디오 사진 등의 파일은 바이너리 파일이라고 부른다.
# 바탕화면에 '매수종목.csv' 파일을 생성한 후 다음과 같이 종목코드와 종목명을 파일에 써보세요. 인코딩은 'cp949'를 사용해야합니다.


# import csv

# c = open("C:\\Users\\user\\Desktop\\programming매수종목.csv", mode = "wt", encoding = 'cp949', newline = "") # 엑셀파일에서 자동 한줄 띄움이 되기 때문에 newline 을 "" 로 지정해준다.
# writer = csv.writer(c)  # 파일의 객체를 가리키는 변수
...
# f = open("C:\\Users\\user\\Desktop\\programming매수종목1.txt", mode = "rt" , encoding = "utf-8") #mode는 적어도 되고 적지 않아도 된다.
# lines = f.readlines()
...
# for i in per:  # 공백 "" 은 float로 변환하면 type error 유발
#     print(float(i))
